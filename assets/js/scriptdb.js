...
let outputhours = document.getElementById("output-hours");
let outputmin = document.getElementById("output-min");
...
submit.addEventListener("click", () => {
  ...
  if (date1.getTime() && date2.getTime()) {
    ...
    var  hours = dayDifference * 24;
    var min = dayDifference *24*60;
    var sec = dayDifference*24*60*60;
     output.innerHTML = `You Have Lived  <span>${dayDifference}</span> days`;
    outputhours.innerHTML = `You Have Lived  <span>${hours}</span> hours`;
    outputmin.innerHTML  = `You Have Lived  <span>${min}</span> minutes`;
    outputsec.innerHTML  = `You Have Lived  <span>${sec}</span> second`;   
  }
